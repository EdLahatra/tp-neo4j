    ...
    return (
      <div className='w-100 flex justify-center'>
        ...
        <div className='w-100' style={{ maxWidth: 400 }}>
          {this.props.data.Personne ? this.props.data.Personne.map((post) =>
            <Post key={post.name} post={post} refresh={() => this.props.data.refetch()} />
          ) : null }
        </div>
      </div>
    )
...
const FeedQuery = gql`query Personne {
  Personne {
    name
  }
}`
